...
DIR = '/home/gn/Code/Archer/tests/c5g7mox2D'
STAT_F = 'rad_c5g7mox2D_mf_hwn.Neutron.output_quantities.stat'
GROUPS = 11
CORES = [48, 96, 240, 384, 480]
lb_time = {}
wall_time = {}
cdofs = {}
HALOS = {}
...
def av_min_max_halos(prefix, cores, groups, max_adapt_step):
    """
    ...
    """
    nodes = np.ndarray(shape=(cores, groups, max_adapt_step))
    halos = np.ndarray(shape=(cores, groups, max_adapt_step))

    # Run through all the partitions and get the number of nodes owned and the number of nodes in the halo 1
    for i in range(cores):
        nodes_temp, halos_temp = np.loadtxt(f'{prefix}.log_{i}', usecols=(1, 3), unpack=True)

        # Takes the loaded file of length (groups*adapts)
        # and converts it from a 1D array to a 2D array [groups, adapts]
        nodes[i] = np.reshape(nodes_temp, (groups, max_adapt_step))
        halos[i] = np.reshape(halos_temp, (groups, max_adapt_step))

    # This will return the average of halos, nodes for a given core,
    # for all group sets and for all adapt steps ==> [groups * adapts]
...
for cpu in CORES:
    ...
    AV[cpu], MIN[cpu], MAX[cpu], NODES[cpu], HALOS[cpu] = av_min_max_halos(
        'rad_c5g7mox2D_mf_hwn', cpu, GROUPS, 5)
